...
print( f'start processing {fname} {elp:.2f}' )
			...
			for i in range(ln):
				...
				try:
					if i in ma or (i+k) in ma:
						...
						C = np.nanmean(M[i0:i1,j0:j1])
						A[i,(i+k)] = C
						A[(i+k),i] = C
				except IndexError: pass
	else: pass
elp = timeit.default_timer() - start_time
print( f'\tresolution combined {elp:.2f}')
print(ma)
A = np.nan_to_num(A)
if args.pic != False:
	im = plt.imshow(A, cmap='coolwarm',vmin = 0, vmax = 2.5)
	plt.colorbar(im)
	plt.savefig(f'{args.pic}/{fname}.smooth.median.png',dpi=400)
	plt.clf()
A = A - np.nanmean(A,axis=0)
if args.pic != False:
	im = plt.imshow(A, cmap='coolwarm',vmin = -2.5, vmax = 2.5)
	plt.colorbar(im)
	plt.savefig(f'{args.pic}/{fname}.norm.mean.png',dpi=400)
	plt.clf()
A = np.nan_to_num(A)
A = np.corrcoef(A)
A[ma,:] = np.nan
...
if args.pic != False:
	...
	plt.colorbar(im)
	plt.savefig(f'{args.pic}/{fname}.smouth.prs.png',dpi=400)
	plt.clf()
S = np.zeros([ln,ln])
P = np.zeros([ln,ln])
for i in range(ln): 
...
elp = timeit.default_timer() - start_time
print( f'\tcorrelating and cleaning {elp:.2f}')
for f in args.enhancing:
	elp = timeit.default_timer() - start_time
	print( f'\t\tstart contracting frame {f} {elp:.2f}' )
	for i in range(ln):
		for j in range(i+1,ln):
			...
			S[j,i] = S[i,j]
	elp = timeit.default_timer() - start_time
	print( f'\t\tend contracting frame {f} {elp:.2f}' )
	if args.pic != False:
		im = plt.imshow(P, cmap='coolwarm',vmin = -2, vmax = 2)
		plt.colorbar(im)
		plt.savefig(f'{args.pic}/{fname}.{f}.ce.prc.prs.png',dpi=400)
		plt.clf()
	np.savetxt(f'{args.out}/{fname}.{f}.ce.prc.prs', P)
	P = np.zeros([ln,ln])
	if args.pic != False:
		im = plt.imshow(S, cmap='coolwarm',vmin = -1, vmax = 1)
		plt.colorbar(im)
		plt.savefig(f'{args.pic}/{fname}.{f}.ce.range.prs.png',dpi=400)
		plt.clf()
	np.savetxt(f'{args.out}/{fname}.{f}.ce.range.prs', S)
	S = np.zeros([ln,ln])
	elp = timeit.default_timer() - start_time
	print( f'\t\t writing contracting matrix frame {f} {elp:.2f}' )
del P
del S
elp = timeit.default_timer() - start_time
print( f'end processing {fname} {elp:.2f}' )
